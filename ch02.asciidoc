...
With adding prepended and appended content to an input, you can add common elements like the dollar symbol, the `@` for a Twitter username or anything else that might be common for your application interface. To use, simply place a `<span>` with an `.add-on` class either before or after an input to append it to the input.
