...
The most common form text field is the input--this is where users will enter most of the essential form data (see <<figure2-23>>). Bootstrap offers support for all native HTML5 input types: text, password, datetime, datetime-local, date, month, time, week, number, email, URL, search, tel, and color(((input types)))(((HTML5 input types)))(((data input))):
...
The `textarea` is used when you need multiple lines of input (see <<figure2-24>>). You'll find you mainly modify the `rows` attribute, changing it to the number of rows that you need to support (fewer rows = smaller box, more rows = bigger box)((("textarea input type))):
