...
For a button, simply add the `disabled` attribute to the button (see <<figure2-46>>). This will actually disable the button, so JavaScript is not directly needed(((range="endofrange", startref="ix_CSSbutt"))):
