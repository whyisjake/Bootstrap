...
A well is a container `<div>` that causes the content to appear sunken on the page (see <<figure3-50>>). I have used wells for blog post meta information like author, date, and categories. To create a well, simply wrap the content that you would like to appear in the well with a `<div>` containing the class of `.well`.(((layout components, wells)))
