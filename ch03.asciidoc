...
Instead of using the default class-based forms from <<css>>, forms that are in the navbar use the `.navbar-form` class. This ensures that the form's margins are properly set and match the nav stylings. Of note, `.pull-left` and `.pull-right` helper classes may help move the form in to the proper position.
