...
Menus can also be built to drop up rather than down (see <<figure3-8>>). To make this change, simply add `.dropup` to the `.btn-group` container. To have the button pull up from the righthand side, add `.pull-right` to the `.dropdown-menu`: (((dropup menus)))(((menus, dropup)))
...
Take note: the caret is now pointed up because the menu will be going up instead of down.
