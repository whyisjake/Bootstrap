...
If you have multiple button groups (see <<figure3-3>>) that you want to place on a single line, wrap multiple `.btn-group` classes with `.btn-toolbar`: For more information about using icons with buttons, follow the examples in <<css>>. Also,  for an example a button toolbar.(((buttons, button toolbars)))((("toolbars, button toolbars"))) 
