...
To add a dropdown to a button, simply wrap a button and dropdown menu in a `.btn-group`. You can also use a `<span class="caret"></span>` to act as an indicator that the button is a dropwdown.
