...
Bootstrap also provides a helper class, `.active`. In principle, it generally adds distinction to the current element and set apart from the rest of the navigation elements. You can add this class to the home page links or add it to the links of the page that the user is currently on.(((active helper class)))
