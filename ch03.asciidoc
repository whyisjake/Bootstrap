...
Like the modals that will be described in <<javascriptplug>>, alerts provide a way to style messages to the user. The default alert is added by creating a wrapper `<div>` and adding a class of `.alert`.
...
The `.alert` uses the alerts jQuery plugin that is discussed in <<javascriptplug>>. To close the alert, you can use a button that contains the `data-dismiss="alert"` attribute. Mobile Safari and Mobile Opera browers require an `href="#"` to close.
