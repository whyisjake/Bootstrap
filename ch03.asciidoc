...
//This needs to be updated so that it shows a divider.
...
Instead of using the deafault, class based forms from chapter 2, forms that are in the navbar use the `.navbar-form` class. This ensures that the forms margins are properly set, and match the nav stylings. Of note, `.pull-left`, and `.pull-right` helper classes can help move the form in the proper position. 
...
.Fixed Top Navbar
image::image/navbar-fixed-top.png[]
...
.Fixed Bottom Navbar
image::image/navbar-fixed-bottom.png[]
...
.Static Top Navbar
image::image/navbar-static-top.png[]
