...
The default progress bar has a light gray background and a blue progress bar. To create it, add a `<div>` with a class of `.progress`. Inside, add an empty `<div>` with a class of `.bar`. Add a style attribute with the width in percentage. In <<figure3-42>>, I added `style="60%";` to indicate that the progress bar was at 60%.
