...
Generally, it is best to include all JavaScript calls into a check that ensures that the DOM has been loaded on the page. As the browser parses the page, if you have the JavaScript trying to fire earlier, it may miss elements. With jQuery, adding a check is easily done by selecting the `document`, or the entire content of the page, an then applying the `.ready()` method.
