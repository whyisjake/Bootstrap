...
There are two events that can be tied to Bootstrap's +alert+ class that you can see in <<table4-8>>.
...
See <<table4-13>> for a list of options.
...
To apply the affix plugin to a `<div>`, you can use either data attributes, or you can use JavaScript directly. Note, you must position the element so that it can be affixed to the page. Position is controlled by the `data-spy` attribute, using either  `affix`, `affix-top`, or `affix-bottom`. You then use the `data-offset` to calculate the position of the scroll.(((range="endofrange", startref="ix_jsplug")))
