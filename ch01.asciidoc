...
You can move columns to the right using the `.offset*` class. Each class moves the span over that width. So an `.offset2` would move a `.span7` over two columns (see <<figure1-2>>)(((columns, offsetting))):
