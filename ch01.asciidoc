...
At the core, Bootstrap supports five different layouts, each relying on CSS media queries. Large displays, where the window width is at least 1200 pixels wide. This layout has columns that are 70 pixels wide each, and gutters that are 30 pixels wide. 
